...
function renderProjects() {
  ...
  grid.innerHTML = PROJECTS.map((p, i) => {
    ...
    // stack: max 4 visible, +N for rest
    const stackHTML = p.stack.slice(0, 4).map(s =>
      '<span class="sp ' + stackColor(s) + '">' + s + '</span>'
    ).join('') + (p.stack.length > 4
      ? '<span class="sp" style="color:var(--text-d);border-color:var(--border)">+' + (p.stack.length - 4) + '</span>'
      : '');
...
function openProjectModal(idx) {
  ...
  if (firstSlide) {
    modalImg.innerHTML = '<img src="' + firstSlide + '" alt="' + p.title.replace(/"/g,'') + '" onerror="this.parentElement.innerHTML=\'<div class=&quot;pc-ph&quot; style=&quot;font-size:5rem&quot;>' + p.emoji + '</div>\'" style="width:100%;height:100%;object-fit:cover;">';
  } else {
...
function switchModalImg(el, src) {
  document.getElementById('modal-main-img').innerHTML = '<img src="' + src + '" alt="" style="width:100%;height:100%;object-fit:cover;">';
